#import "SSPrefsController.h"
#import "SSPreferencePaneProtocol.h"

@implementation SSPrefsController
...
{
  return [[[SSPrefsController alloc] initWithPanesSearchPath:path bundleExtension:ext] autorelease];
}


+ (id)preferencesWithBundleExtension:(NSString *)ext
{
  return [[[SSPrefsController alloc] initWithBundleExtension:ext] autorelease];
}


+ (id)preferencesWithPanesSearchPath:(NSString*)path
{
  return [[[SSPrefsController alloc] initWithPanesSearchPath:path] autorelease];
}


+ (id)preferences
{
  return [[[SSPrefsController alloc] init] autorelease];
}
  ...
  if (paneBundle) {
    ...
    if (paneName) {
      ...
      if (!paneClass) {
        ...
        if ([paneClass conformsToProtocol:@protocol(SSPreferencePaneProtocol)] && [paneClass isKindOfClass:[NSObject class]]) {
          NSArray *panes = [paneClass preferencePanes];
          
          NSEnumerator *enumerator = [panes objectEnumerator];
          id <SSPreferencePaneProtocol> aPane;
...
- (BOOL)loadPrefsPaneNamed:(NSString *)name display:(BOOL)disp
{
  ...
  id <SSPreferencePaneProtocol> pane = [preferencePanes objectForKey:name];
